...
class authController {
    async registration(req, res) {
        try {
            ...
            const counter = new Counter({title: 'Income counter', typeincome: 'Income', timestamp: 0, income: 0, period: 0, output: 0})
            const user = new User({username, password: hashPassword, roles: [userRole.value], counters: [{counter}]})
            await user.save()
    ...
    async createCounter(req, res) {
        try {
            const {username, title, typeincome, timestamp, income, period, output} = req.body
            const counter = new Counter({title: `${title}`, typeincome: `${typeincome}`, timestamp: `${timestamp}`, income: `${income}`, period: `${period}`, output: `${output}`})
            const user = await User.findOne({username})
            if (user) {
